...
export function skillAssessments(employee: Employee, data: Dataset) {
  const values = { ...employee.skills };
  const observations = new Map((data.approved_assessments ?? []).filter(a => a.employeeId === employee.employee_id).map(a => [a.skillId, a]));
  for (const observation of observations.values()) values[observation.skillId] = observation.value;
  return { values, observations };
}

export function effectiveSkills(employee: Employee, data: Dataset) {
  const { values: skills, observations } = skillAssessments(employee, data);
  ...
  const records = data.history.filter(row => !demoIds.has(row.record_id) && row.employee_id === employee.employee_id && row.status === "completed" && row.date <= data.meta.as_of_date)
  ...
  for (const record of records) {
    for (const gain of events.get(record.event_id)?.develops_skills ?? []) {
      if (record.date <= (observations.get(gain.skill_id)?.evidenceAsOf ?? employee.last_review_date)) continue;
  ...
  for (const completion of data.demo_completions ?? []) {
    ...
    for (const gain of completion.gains) {
      if (observations.get(gain.skill_id)?.includedDemoIds.includes(completion.id)) continue;
      const old = skills[gain.skill_id] ?? 0;
      skills[gain.skill_id] = Math.max(old, Math.min(5, old + gain.gain, gain.max_level));
    }
  }
...
export function development(employee: Employee, data: Dataset) {
  ...
  const { values: assessed } = skillAssessments(employee, data);
  ...
  const gaps = Object.entries(profile?.required_skills ?? {}).map(([id, required]) => ({
    ...
    assessed: assessed[id] ?? 0, closed: (assessed[id] ?? 0) >= required,
