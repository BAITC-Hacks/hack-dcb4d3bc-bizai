...
export const milestoneSchema = z.object({ id: z.string().min(1).max(100), goalId: z.string(), outcome: text, criterion: text, state: z.enum(["planned", "in_progress", "evidence_needed", "reached", "blocked", "paused"]), evidence: z.string().max(4000), actions: z.string().max(4000), activityIds: z.array(z.string()).max(30).optional(), origin: z.literal("employee") });
...
export function validatePlan(plan: Plan, data: Dataset) {
  ...
  for (const milestone of plan.milestones) if (milestone.activityIds?.some(id => !data.events.some(event => event.event_id === id))) throw new Error("Unknown activity reference");
