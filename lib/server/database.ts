import { canReadReview, quarterPeriod, validateReviewItems, type ReviewCycle, type ReviewItem } from "../career/reviews";
...
import { effectiveSkills } from "../career/skills";
import { randomUUID } from "node:crypto";
import { DatabaseSync } from "node:sqlite";
import { mkdirSync, readFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { parseDataset, mergeImport } from "../career/import";
import type { Dataset, Snapshot, DemoCompletion } from "../career/types";
...
export class SqliteRepository {
  ...
  updateReview(actor: Actor, id: string, expectedRevision: number, datasetRevision: number, action: "save" | "submit" | "reopen", items: ReviewItem[]): ReviewCycle {
    ...
    try {
      ...
      if ((action === "reopen" && current.status !== "submitted") || (action !== "reopen" && current.status !== "draft")) throw new Error("Invalid review transition");
