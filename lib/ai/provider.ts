...
export const promptVersion = "career-assistant-v4";
...
When ready, select 1–3 items ONLY from candidate_ids, prioritizing critical target gaps, then useful gains and effort. Each recommendation identifies event_id and one skill_id from that event's contributions. The server attaches saved-goal/event/gap evidence automatically; evidence_ids may add other relevant supplied facts such as participation. Use the per-event participation evidence, including failures in activities sharing skills, when comparing alternatives. Repeated no-shows or drops are an observed feasibility concern, not proof of a preference: weigh them against critical target requirements and confirmed constraints; ask a focused question when they could change the decision. Compare a real alternative in insights. Every recommendation will display current grade, target requirements, projected gap improvement, and related participation. A ready coach response with no questions MUST contain 1–3 recommendations; an empty successful answer is invalid. An eligible event whose cap cannot help the gap is not a useful recommendation. Do not repeat a saved goal as a goal_draft unless the employee requests a change.
