...
const command = z.object({ commandId: z.string().uuid(), eventId: z.string().min(1).max(100), datasetRevision: z.number().int().positive(), planRevision: z.number().int().nonnegative() });
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json(repository().completeActivity(actor, input.eventId, input.datasetRevision, input.planRevision, input.commandId));
  } catch (error) {
