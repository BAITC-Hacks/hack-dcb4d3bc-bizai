...
export default async function Page({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <><h1 className="flex items-center gap-3"><GraduationCap className="h-8 w-8 text-brand-navy"/>Development activities</h1><p className="text-muted-foreground">Eligibility uses your current role and grade. Mandatory records stay visible but are excluded from voluntary recommendations.</p>
    <div className="grid gap-4 sm:grid-cols-3">{[["Completed activities",completed.size],["In progress",active.size],["Eligible to explore",eligible.size]].map(([label,value]) => <Card key={label}><CardContent className="p-5"><p className="text-xs uppercase tracking-widest text-muted-foreground">{label}</p><p className="mt-2 text-3xl font-semibold text-brand-navy">{value}</p></CardContent></Card>)}</div>
    <nav aria-label="Activity filters" className="flex flex-wrap gap-1 rounded-xl bg-brand-mist/60 p-1">{[["all","All activities"],["eligible","Eligible"],["in_progress","In progress"],["completed","Completed"],["mandatory","Mandatory"]].map(([value,label]) => <Link key={value} href={`/employee/learning?tab=${value}`} aria-current={tab === value ? "page" : undefined} className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === value ? "bg-white text-brand-navy shadow-sm" : "text-muted-foreground hover:bg-white/60"}`}>{label}</Link>)}</nav>
    {!filtered.length && <div className="rounded-2xl border border-dashed bg-white p-10 text-center text-muted-foreground">No activities in this category.</div>}
