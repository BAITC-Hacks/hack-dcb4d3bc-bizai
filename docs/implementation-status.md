...
## Advisor chat workspace and cross-page widget

Replaced the form-style advisor with a dedicated chat surface: prominent header, continuous user/assistant message history, starter prompts, a bottom composer, Enter/Shift+Enter behavior, copy, retry, Stop, auto-scroll/latest-message controls and a context/preferences drawer. The same component powers a floating widget across employee and HR workspaces. Widget conversation survives ordinary navigation; full chat uses the same stored history. HR must select an employee. The launcher is hidden on full advisor pages, and no personal chat appears before account selection.
