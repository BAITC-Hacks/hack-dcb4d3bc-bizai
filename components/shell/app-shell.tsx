...
export async function AppShell({ role, children }: { role: "hr" | "employee"; children: React.ReactNode }) {
  ...
  return <WorkspaceShell role={role} name={employee?.full_name ?? t("HR workspace")} position={employee ? `${t(employee.grade)} · ${t(employee.role)}` : t("People & development")} date={data.meta.as_of_date}>{children}</WorkspaceShell>;
}
