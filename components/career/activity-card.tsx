...
const conciseReason: Record<string, string> = {
  ...
  "Outside current and chosen target audiences": "Outside current and chosen target audiences",
...
export async function ActivityCard({ event, reasons, skillNames, date, targetAccess = false }: {
  event: Event; reasons: string[]; skillNames: Map<string, string>; date: string; targetAccess?: boolean;
}) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-t-xl border bg-white">
      {targetAccess && <p className="rounded-t-xl bg-blue-50 px-4 py-2 text-xs text-brand-navy">{t(eligible ? "Available through your chosen target" : "Matches your chosen target audience")} · {event.target_roles.map(t).join(", ")} / {event.target_grades.map(t).join(", ")}</p>}
      <header className="relative border-b px-5 pb-4 pt-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-100 text-brand-navy"><Icon className="h-4 w-4" aria-hidden="true"/></span>
      ...
      <div className="flex flex-1 flex-col gap-3 px-4 py-3">
        ...
        <details><summary className="cursor-pointer text-xs font-medium text-brand-navy">{t("Requirements and skill gains")}</summary><section aria-label={t("Skill gains")} className="rounded-lg border border-brand-gold/20 bg-brand-gold/[0.04] px-3 py-2.5">
        ...
        </details><section aria-label={t("Sessions")} className="mt-auto border-t border-dashed pt-2.5">
