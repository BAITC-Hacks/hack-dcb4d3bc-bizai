...
export function AssistantPanel({ employeeId, mode, activities = [], compact = false }: { employeeId: string; mode: AssistantMode; activities?: { id: string; title: string }[]; compact?: boolean }) {
  ...
  function renderAnswer(turn: AssistantResult) {
    ...
    return <div className="space-y-3">
      ...
      {turn.advice.recommendations.map(item => <div key={item.event_id} className="rounded-xl border border-brand-gold/30 p-3"><p className="text-sm font-semibold">{t(turn.activities.find(a => a.id === item.event_id)?.title) || item.event_id}</p><p className="mt-1 text-sm leading-relaxed">{item.reason}</p>{mode !== "hr" && <a className="mt-3 inline-flex text-sm font-semibold text-brand-navy underline" href={`/employee/learning?event=${encodeURIComponent(item.event_id)}`}>{t("Explore this activity")} →</a>}<p className="mt-2 text-[10px] text-muted-foreground">{say("sources")}: {item.evidence_ids.map(id => { const fact = turn.evidence.find(f => f.id === id); return fact ? evidenceLabel(fact.id, fact.label) : t("Supporting record"); }).join(" · ")}</p></div>)}
      ...
      <details><summary className="cursor-pointer text-[10px] text-muted-foreground">{t("Technical details")}</summary><p className="text-[10px] text-muted-foreground">{turn.model ?? "rules"} · {formatNumber(turn.latencyMs / 1000, 1)} {t("s")} · {turn.id.slice(0, 8)}</p></details>
    </div>;
  }
