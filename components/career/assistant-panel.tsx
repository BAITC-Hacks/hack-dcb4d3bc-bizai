...
export function AssistantPanel({ employeeId, mode, activities = [], compact = false }: { employeeId: string; mode: AssistantMode; activities?: { id: string; title: string }[]; compact?: boolean }) {
  ...
  function renderAnswer(turn: AssistantResult) {
    ...
    return <div className="space-y-3">
      ...
      {turn.advice.questions.map((question, i) => <p key={i} className="border-l-2 border-teal-600 pl-3 text-sm font-medium leading-6">{question}</p>)}
      ...
      <details><summary className="cursor-pointer text-[10px] text-muted-foreground">{t("Technical details")}</summary><p className="text-[10px] text-muted-foreground">{say(turn.engine === "openai" ? "interpretation" : "rules")} · {say(turn.state)} · {turn.model ?? "rules"} · {formatNumber(turn.latencyMs / 1000, 1)} {t("s")} · {turn.id.slice(0, 8)}</p></details>
    </div>;
  }
