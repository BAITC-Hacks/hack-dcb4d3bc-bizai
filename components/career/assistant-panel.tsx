...
import { PreferenceProposal } from "./preference-proposal";
...
import { Sparkles, ArrowUp, Square, SlidersHorizontal, Copy, Check, ArrowDown, ArrowUpRight, X } from "lucide-react";
...
export function AssistantPanel({ employeeId, mode, activities = [], compact = false }: { employeeId: string; mode: AssistantMode; activities?: { id: string; title: string }[]; compact?: boolean }) {
  ...
  function renderAnswer(turn: AssistantResult) {
    const currentPreferences = (turn.consultationRevision ?? 0) === session?.consultation.revision;
    const factIds = new Set([...(turn.advice.summary_evidence_ids ?? []), ...[...turn.advice.insights, ...turn.advice.recommendations].flatMap(item => item.evidence_ids)]);
    ...
    return <div className="space-y-3">
      ...
      {!!turn.advice.insights.length && <details className="chat-disclosure" open={!turn.advice.recommendations.length && !turn.advice.questions.length}><summary>{say("chatContextFacts")}</summary><ul className="mt-3 space-y-3 text-sm leading-relaxed">{turn.advice.insights.map((item, i) => <li key={i} className="whitespace-pre-wrap">{item.text}</li>)}</ul></details>}
      {!currentPreferences && <p className="text-xs text-muted-foreground">{say("previousPreferences")}</p>}
      {currentPreferences && turn.advice.questions.map((question, i) => <p key={i} className="border-l-2 border-teal-600 pl-3 text-sm font-medium leading-6">{question}</p>)}
      {currentPreferences && turn.advice.goal_draft && <div className="rounded-xl border bg-brand-paper p-3"><p className="text-xs text-muted-foreground">{say("draft")}</p><p className="mt-1 text-sm">{turn.advice.goal_draft.wording}</p>{mode !== "hr" && <Button type="button" className="mt-3" variant="outline" disabled={busy || turn.id !== latest?.id} onClick={() => accept(turn)}>{say("accept")}</Button>}</div>}
      {currentPreferences && turn.advice.recommendations.map(item => <div key={item.event_id} className="chat-recommendation"><p className="text-sm font-semibold">{t(turn.activities.find(a => a.id === item.event_id)?.title) || item.event_id}</p><details className="chat-disclosure mt-2"><summary>{say("chatWhy")}</summary><p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed">{item.reason}</p><p className="mt-2 text-[11px] text-muted-foreground">{say("sources")}: {item.evidence_ids.map(id => { const fact = turn.evidence.find(f => f.id === id); return fact ? evidenceLabel(fact.id, fact.label) : t("Supporting record"); }).join(" · ")}</p></details>{mode !== "hr" && <a className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-teal-800 hover:underline" href={`/employee/learning?event=${encodeURIComponent(item.event_id)}`}>{t("Explore this activity")} <ArrowUpRight size={13}/></a>}</div>)}
      {currentPreferences && turn.id === latest?.id && turn.advice.consultation_draft && mode !== "hr" && session && <PreferenceProposal draft={turn.advice.consultation_draft} revision={session.consultation.revision} datasetRevision={session.datasetRevision} planRevision={session.planRevision} busy={busy} onBusy={setBusy} onSaved={() => { setNotice("preferencesSaved"); router.refresh(); setReload(n => n + 1); announce(); }}/>}
      {!!facts.length && <details className="chat-disclosure"><summary>{say("sources")} · {facts.length}</summary><div className="mt-3 space-y-3">{facts.map(fact => <div key={fact.id} className="min-w-0 border-t pt-2"><p className="text-xs font-semibold">{evidenceLabel(fact.id, fact.label)}</p><p className="mt-1 text-xs">{evidenceSummary(fact)}</p><details className="mt-2"><summary className="cursor-pointer text-[10px] text-muted-foreground">{t("Technical details")}</summary><p className="break-all text-[10px] text-muted-foreground">{fact.id} · {say("source")}: {fact.source}</p><pre className="mt-1 max-h-48 overflow-auto whitespace-pre-wrap break-words rounded bg-brand-paper p-2 text-[10px]">{JSON.stringify(fact.value, null, 2)}</pre></details></div>)}</div></details>}
      <details><summary className="cursor-pointer text-[10px] text-muted-foreground">{t("Technical details")}</summary><p className="text-[10px] text-muted-foreground">{say(turn.engine === "openai" ? "interpretation" : "rules")} · {say(turn.state)} · {turn.model ?? "rules"} · {formatNumber(turn.latencyMs / 1000, 1)} {t("s")} · {turn.id.slice(0, 8)}</p>{!!turn.toolCalls?.length && <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap break-words text-[10px] text-muted-foreground">{JSON.stringify(turn.toolCalls, null, 2)}</pre>}</details>
    </div>;
  }
  ...
  return <section id={compact ? undefined : "assistant"} aria-label={title} className={`chat-workspace ${compact ? "chat-workspace-compact" : ""}`}>
    <header className="chat-header"><div className="chat-avatar"><Sparkles size={20}/></div><div className="min-w-0 flex-1"><h2 className="text-sm font-semibold text-slate-800">{title}</h2><p className="chat-subtitle mt-0.5 text-xs text-muted-foreground">{say("chatGrounded")}</p></div><button type="button" className="chat-header-button" aria-label={say("chatContext")} title={say("chatContext")} aria-expanded={contextOpen} onClick={() => setContextOpen(v => !v)}><SlidersHorizontal size={18}/></button></header>
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      <div ref={transcript} role="log" aria-label={say("chatConversation")} aria-live="polite" aria-relevant="additions" className="chat-transcript" onScroll={() => { const el = transcript.current!; follow.current = el.scrollHeight - el.scrollTop - el.clientHeight < 100; setAtBottom(follow.current); }}>
        ...
        {session && !session.turns.length && !pendingMessage && <div className="chat-welcome"><h3 className="text-xl font-semibold tracking-tight">{say("chatWelcome")}</h3><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted-foreground">{say("description")}</p><div className="mt-5 flex flex-wrap justify-center gap-2">{starters.map((key, i) => <button key={key} type="button" disabled={busy} className="chat-starter" onClick={() => ask(say(key))}><span>{say(mode === "coach" ? (["chatNext", "chatGaps", "chatGoal"] as const)[i] : "start")}</span><ArrowUpRight size={16}/></button>)}</div></div>}
        {session?.turns.map(turn => <div key={turn.id} className="chat-turn"><div className="chat-user-row"><div className="chat-user-bubble"><span className="sr-only">{say("chatYou")}: </span>{turn.message}</div></div><div className="chat-assistant-row"><span className="chat-bot-avatar" aria-hidden="true"><Sparkles size={16}/></span><article className="chat-answer">{renderAnswer(turn)}<button type="button" className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground hover:text-brand-navy" onClick={() => copy(turn)}>{copied === turn.id ? <Check size={13}/> : <Copy size={13}/>}{say(copied === turn.id ? "chatCopied" : "chatCopy")}</button></article></div></div>)}
        {pendingMessage && <div className="chat-turn"><div className="chat-user-row"><div className="chat-user-bubble">{pendingMessage}</div></div><div className="chat-assistant-row"><span className="chat-bot-avatar"><Sparkles size={16}/></span><div className="chat-answer">{streamText ? <p className="whitespace-pre-wrap text-sm leading-6">{streamText}<span className="chat-cursor"/></p> : sending ? <div role="status" className="flex items-center gap-3 text-xs text-muted-foreground"><span className="chat-typing" aria-hidden="true"><i/><i/><i/></span>{say(phase)}</div> : <p className="text-xs text-muted-foreground">{say(notice === "stopped" ? "stopped" : "chatInterrupted")}</p>}</div></div></div>}
      </div>
    ...
    <footer className="chat-footer">
      {session && mode !== "hr" && !session.consultation.answers && latest?.reason && <button type="button" className="mb-3 flex items-center gap-2 text-left text-xs font-medium text-teal-800" onClick={() => setContextOpen(true)}><SlidersHorizontal size={14}/>{say("chatPreferences")}<ArrowUpRight size={13}/></button>}
      ...
      <p className="mt-2 text-center text-[10px] text-muted-foreground">{say("chatHintShort")}</p>
    </footer>
  </section>;
}
