...
type Requirement = { id: string; name: string; level: number; required: number; gap: number; critical: boolean };
...
export function DevelopmentFlow({ current, target, gaps, goalUnset }: { current: { role: string; grade: string }; target: { role: string; grade: string }; gaps: Requirement[]; goalUnset: boolean }) {
  ...
  const met = gaps.filter(gap => gap.gap === 0);
  const groups = [
    { title: "Critical gaps", description: "Essential requirements to develop", icon: ShieldAlert, skills: gaps.filter(gap => gap.critical && gap.gap > 0), color: "text-amber-800", background: "bg-amber-50", border: "border-amber-200", bar: "bg-amber-500" },
    { title: "Skills to develop", description: "Other requirements to develop", icon: TrendingUp, skills: gaps.filter(gap => !gap.critical && gap.gap > 0), color: "text-brand-navy", background: "bg-blue-50", border: "border-blue-200", bar: "bg-brand-navy" },
    { title: "Requirements met", description: "Strengths already at target", icon: CheckCircle2, skills: met, color: "text-emerald-800", background: "bg-emerald-50", border: "border-emerald-200", bar: "bg-emerald-600" },
  ];

  return <section id="trajectory" className="overflow-hidden rounded-2xl border bg-white shadow-card">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b p-5"><div><h2 className="flex items-center gap-2 text-lg font-semibold"><GitBranch aria-hidden="true" className="h-5 w-5 text-brand-gold" />{t("Development map")}</h2><p className="mt-1 text-xs text-muted-foreground">{t("Branches show target requirements, not a sequence of tasks.")}</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800">{t("Requirements met")}: {formatNumber(met.length)} / {formatNumber(gaps.length)}</span></div>
      ...
      <div className="grid items-start gap-4 md:grid-cols-[1fr_2fr_1fr]">
        {groups.map(group => <section key={group.title} className={`min-w-0 overflow-hidden rounded-xl border bg-white ${group.border}`}>
          ...
          <ul className={`px-4 ${group.title === "Skills to develop" ? "grid gap-x-4 sm:grid-cols-2" : ""}`}>{group.skills.map(skill => <li key={skill.id} className="border-b border-slate-100 py-3 last:border-b-0">
            ...
            <div className="mt-1.5 flex justify-between gap-2 text-[10px]"><span className={group.color}>{skill.gap ? `${t("Gap")}: ${formatNumber(skill.gap, 1)}` : t("Met")}</span>{skill.critical && <span className="font-semibold text-amber-800">{t("Critical")}</span>}</div>
          </li>)}</ul>
    ...
    <div className="flex flex-wrap justify-between gap-2 border-t px-5 py-3 text-xs text-muted-foreground"><span>{t("All target skills, grouped by priority. Levels show current / required.")}</span><a href="#skills" className="font-semibold text-brand-navy underline underline-offset-4">{t("Skills against target")}</a></div>
  </section>;
}
