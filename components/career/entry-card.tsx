...
export function EntryCard({ employees }: { employees: { id: string; name: string }[] }) {
  ...
  return <div className="overflow-hidden rounded-2xl bg-white shadow-elevated">
    <div className="gradient-navy p-6 text-white"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-widest text-white/60">{t("Enter the portal")}</p><h2 className="mt-1 text-2xl text-white">{t("Choose your workspace")}</h2></div><Building2 className="h-8 w-8 text-brand-gold-light"/></div>
