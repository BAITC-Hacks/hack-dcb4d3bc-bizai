...
export async function DevelopmentWorkspace({ employee, data, planning, revision, base }: { employee: Employee; data: Dataset; planning: PlanningState; revision: number; base: string }) {
  ...
  const next = !goal ? [t("Set your direction"), t("Start with an outcome you care about. A job title is optional."), t("Create a goal"), "plan"] : needsEvidence ? [active?.state === t("evidence_needed") ? t("Review the evidence for your milestone") : t("Learning done. What can you demonstrate?"), t("Review your milestone success criterion and add evidence of the outcome. Completion alone does not finish the milestone."), t("Review my outcome"), "plan"] : active ? [t("Continue your next milestone"), active.outcome, t("Open my plan"), "plan"] : stale ? [t("Your plan changed"), t("Refresh your advice before choosing a next step."), t("Refresh advice"), "advisor"] : !consultation.answers ? [t("Make the next step fit your life"), t("Confirm your available time and preferred learning format."), t("Set my preferences"), "advisor"] : recommendations.length ? [t("Your next step is ready to explore"), t("Inspect the activity and its evidence before adding it to your plan."), t("Review advice"), "advisor"] : [t("Choose a useful next step"), t("Work with your advisor or define an action yourself."), t("Explore with advisor"), "advisor"];
  ...
  return <div className="journey-workspace">
    ...
    <div className="journey-columns"><div className="space-y-6">
      <section className="journey-next"><span className="journey-kicker"><span/> {t("NEXT ACTION")}</span><h2>{next[0]}</h2><p>{next[1]}</p><Link href={`${base}?view=${next[3]}`} className="journey-button">{next[2]}<ArrowRight size={17}/></Link></section>
