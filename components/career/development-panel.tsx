...
import { grades, type Dataset, type Employee } from "@/lib/career/types";
...
export async function DevelopmentPanel({ employee, data, view = "overview" }: { employee: Employee; data: Dataset; view?: string }) {
  ...
  const suggestedTarget = !focusedGoal && nextGrade && data.role_profiles.some(p => p.role === employee.role && p.grade === nextGrade) ? { target_role: employee.role, target_grade: nextGrade } : null;
  const preview = suggestedTarget ? development({ ...employee, career_goal: suggestedTarget }, data) : null;
  ...
  return <>
    ...
    {active === "overview" && <>
      {preview && <div className="context-strip"><span>{t("Optional next-grade preview")}: {t(suggestedTarget!.target_role)} · {t(suggestedTarget!.target_grade)}</span><Link className="ml-auto underline" href={href("skills")}>{t("Explore requirements")}</Link></div>}
      <DevelopmentWorkspace employee={employee} data={data} planning={planning} revision={snapshot.revision} base={base}/>
    </>}
    {active === "plan" && <div className="workspace-content"><p className="page-description">{t("Define outcomes, actions and success criteria.")}</p><PlanningEditor suggestedTarget={suggestedTarget} employeeId={employee.employee_id} key={`${employee.employee_id}-${planning.revision}`} initial={planning} datasetRevision={snapshot.revision} profiles={data.role_profiles} editable={!isHR && actor?.employeeId === employee.employee_id}/></div>}
    ...
    {active === "skills" && <>{preview && <section className="space-y-3"><div className="context-strip"><span>{t("Optional next-grade preview")}. {t("This preview does not change your saved goal.")}</span><Link className="ml-auto underline" href={href("plan")}>{t("Draft next-grade goal")}</Link></div><DevelopmentMap employee={{ ...employee, career_goal: suggestedTarget }} data={data} base={base}/></section>}<div className="context-strip">{result.target ? `${t(result.target.target_role)} · ${t(result.target.target_grade)} · ${result.closed}/${result.gaps.length} ${t("Closed modules")}` : t("Choose a target to compare requirements")}</div>{result.target && <DevelopmentMap employee={{ ...employee, career_goal: focusTarget(planning.plan) }} data={data} base={base}/>}<SkillCards key={`${employee.employee_id}-${planning.revision}`} skills={data.skills.map(skill => {
