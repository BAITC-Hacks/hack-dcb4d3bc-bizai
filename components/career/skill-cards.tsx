...
export type SkillCard = { id: string; name: string; level: number; assessed: number; assessmentRecorded: boolean; required: number | null; critical: boolean; closed: boolean; inProgress: boolean };
export function SkillCards({ skills, goal, assessmentDate }: { skills: SkillCard[]; goal: string | null; assessmentDate: string }) {
  ...
  return <section id="skills" className="overflow-hidden rounded-2xl border bg-white shadow-card">
    ...
    </div><p className="border-t px-5 py-3 text-xs leading-relaxed text-muted-foreground">{t("Assessment:")} {formatDate(assessmentDate)} · {t("Gold outline marks the target level. In progress means a recorded active learning activity; it does not grant skill gains.")}</p>
  </section>;
}
